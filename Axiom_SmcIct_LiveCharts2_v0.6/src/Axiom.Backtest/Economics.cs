...
internal static class Economics
{
    ...
    public static double Spread(double price, double tick, EconomicsConfig e)
    {
        if (e.SpreadTicks > 0) return e.SpreadTicks * tick;
        if (e.SpreadBps > 0) return price * (e.SpreadBps / 10000.0);
        return 0.0;
    }

    public static double Slippage(double price, double tick, EconomicsConfig e)
    {
        if (e.SlippageTicks > 0) return e.SlippageTicks * tick;
        if (e.SlippageBps > 0) return price * (e.SlippageBps / 10000.0);
        return 0.0;
    }
    ...
    public static double FillEntry(double midPrice, Side side, double tick, EconomicsConfig e)
    {
        // Symmetric half-spread, plus adverse slippage
        var spr = Spread(midPrice, tick, e);
        var slip = Slippage(midPrice, tick, e);
        if (side == Side.Buy) return midPrice + spr / 2.0 + slip;
        return midPrice - spr / 2.0 - slip;
    }

    public static double FillExit(double midPrice, Side side, double tick, EconomicsConfig e)
    {
