...
public sealed class SmcIctEngine
{
    ...
    public AnalysisResult Analyze(IReadOnlyList<Candle> candles, string symbol = "SYMBOL", string tf = Timeframes.M5)
    {
        ...
        var internalSwings = Structure.PivotSwings(entry, intLen);
        var externalSwings = Structure.PivotSwings(entry, extLen);

        // last internal swing levels per bar (for sweeps)
        ...
        var lastIntLow  = new double?[entry.Count];
        {
            ...
            {
                while (ptr < internalSwings.Count && internalSwings[ptr].Index <= i)
                {
        ...
        foreach (var s in internalSwings)
        {
            if (s.IsHigh)
            {
                if (lastHigh is not null && Math.Abs(s.Price - lastHigh.Value) <= eqTol)
                {
                    ...
                    if (lastEqh is null || Math.Abs(lvl - lastEqh.Value) > eqMinSep)
                    {
                        lastEqh = lvl;
                        zonesOut.Add(new Zone(Guid.NewGuid(), ZoneType.EQH, Timeframes.M15, s.Time, null,
            ...
            else
            {
                if (lastLow is not null && Math.Abs(s.Price - lastLow.Value) <= eqTol)
                {
                    ...
                    if (lastEql is null || Math.Abs(lvl - lastEql.Value) > eqMinSep)
                    {
                        lastEql = lvl;
                        zonesOut.Add(new Zone(Guid.NewGuid(), ZoneType.EQL, Timeframes.M15, s.Time, null,
        ...
        // -------- External trend from external structure breaks
        int extPtr = 0;
        double? extHigh = null, extLow = null;
        int extTrend = 0; // +1 bullish, -1 bearish
        ...
        // -------- Sweeps (inducement tracking)
        int? lastSweepUp = null, lastSweepDn = null;

        // -------- Zones with lifecycle
        var zoneStates = new List<ZoneState>();

        bool wantOB = _p.EntryModel is "Any" or "OB";
        bool wantFVG50 = _p.EntryModel is "Any" or "FVG50";
        bool wantBreaker = _p.EntryModel is "Any" or "Breaker";

        for (int i = 0; i < entry.Count; i++)
        {
            // update external swings up to i
            while (extPtr < externalSwings.Count && externalSwings[extPtr].Index <= i)
            {
            ...
            // Dealing range zone (external high/low)
            if (i == extLen) // emit once once structure exists
            {
                zonesOut.Add(new Zone(Guid.NewGuid(), ZoneType.DealingRange, Timeframes.M15, c.Time, null,
                    extLow.Value, extHigh.Value, true, $"DR; mid={(extLow.Value + extHigh.Value)/2:g}; status=Active"));
            }
            ...
            // Optional discount/premium gating based on external dealing range
            bool pdOk = true;
            if (_p.RequireDiscountPremium)
            {
                double mid = (extHigh.Value + extLow.Value) / 2.0;
                if (extTrend == +1) pdOk = c.Close <= mid;
                else if (extTrend == -1) pdOk = c.Close >= mid;
            }

            // Create zones on displacement + BOS
            if (disp && (bosUp || bosDn) && hasInducement && pdOk)
            {
                if (bosUp)
                {
                    if (wantOB) TryCreateOb(entry, zoneStates, i, bullish: true, lookback: _p.ObLookbackBars, useBody: _p.ObUseBody);
                    TryCreateFvg(entry, zoneStates, i, bullish: true, requireDisp: _p.FvgRequireDisplacement, disp: disp);

                    if (zoneStates.Count > 0) zoneStates[^1].SetInduced("disp+bos_bull");
                }
                else
                {
                    if (wantOB) TryCreateOb(entry, zoneStates, i, bullish: false, lookback: _p.ObLookbackBars, useBody: _p.ObUseBody);
                    TryCreateFvg(entry, zoneStates, i, bullish: false, requireDisp: _p.FvgRequireDisplacement, disp: disp);

                    if (zoneStates.Count > 0) zoneStates[^1].SetInduced("disp+bos_bear");
                }
            }

            MaybeCreateBreakers(zoneStates, c, i);

            // Emit mitigation entries + v0.5 SL/TP model
            EmitMitigationSignals(
                _p,
                zonesOut,
                byDay,
                zoneStates, sigsOut, c,
                extHigh: extHigh.Value,
                extLow: extLow.Value,
                tick: tick,
                wantOB: wantOB,
                wantFVG50: wantFVG50,
                wantBreaker: wantBreaker,
                requireInduced: _p.RequireInducement,
                requireMicroConfirm: _p.RequireMicroConfirm,
                microLastByEntryBucket: microLastByEntryBucket
            );
        }
    ...
    // ---------------- Signals (mitigation + trade model) ----------------
        ...
        Dictionary<DateTime, Candle>? microLastByEntryBucket)
    {
        foreach (var z in zones)
        {
            ...
            // FVG50 rule: require mitigation to cross the mid
            ...
            bool ok = z.Zone.IsBullish ? c.Close >= c.Open : c.Close <= c.Open;
            if (!ok) continue;

            var side = z.Zone.IsBullish ? Side.Buy : Side.Sell;
            var entry = c.Close;

            // Pools and SL/TP model
            var pools = TradeModel.CollectPools(p, side, entry, alreadyZones, c.Time, byDay, extHigh, extLow);
