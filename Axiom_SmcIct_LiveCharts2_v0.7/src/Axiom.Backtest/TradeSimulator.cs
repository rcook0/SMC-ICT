...
public static class TradeSimulator
{
        ...
        BacktestConfig cfg)
    {
        ...
        var atr = AtrSeries(baseCandles, Math.Max(2, econ.AtrLen));
        ...
        for (int i = 0; i < baseCandles.Count; i++)
        {
            ...
            if (pos is null)
            {
                ...
                if (iSig < sigs.Count && sigs[iSig].Time == c.Time)
                {
                    ...
                    var entryFill = Economics.FillEntry(midEntry, s.Side, tick, econ, atr[i]);
...
private static double[] AtrSeries(IReadOnlyList<Candle> bars, int len)
{
    var outAtr = new double[bars.Count];
    if (bars.Count == 0) return outAtr;
    ...
    {
        double h = bars[i].High, l = bars[i].Low;
        double a = h - l;
        double b = Math.Abs(h - prevClose);
        double c = Math.Abs(l - prevClose);
